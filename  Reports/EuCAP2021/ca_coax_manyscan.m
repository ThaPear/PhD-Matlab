...
folder = sprintf('%s/ Reports/EuCAP2021/%s/', resultdir_cst, mfilename);
touchstonefolder = sprintf('%s/ Reports/EuCAP2021/%s/', resultdir_matlab, mfilename);
[~, ~] = mkdir(folder);
...
%     [figZ, axZ] = a_figZ(7, fs, zfeed);
    [figVSWR, axVSWR] = a_figVSWR(floor((iangle-1)/7)+1);
%         axVSWR.ColorOrder = [0 0 0];
%         axVSWR.LineStyleOrder = {'-','--',':', '-.'};
...
%         plot(axZ, fs/1e9, imag(ZasC22), '--');
    ...
    efficiency22 = (1-abs(Gamma22).^2);
    if(iangle == 1)
        % Plot efficiency of H/E th60.
        [figEff, axEff] = a_fig(4);
            axEff.ColorOrder = [0 0 0];
            axEff.LineStyleOrder = {'-','--',':', '-.'};
        plot(axEff, fs./1e9, 100*efficiency11);
    ...
    if(iangle == 7)
        ...
        [figEff, axEff] = a_fig(4);
        plot(axEff, fs./1e9, 100*efficiency11);
        addlegendentry('H-plane 60\circ');
        plot(axEff, fs./1e9, 100*efficiency22);
        addlegendentry('E-plane 60\circ');
    end
    if(iangle == 14)
        % Plot efficiency of D th60.
        [figEff, axEff] = a_fig(4);
        plot(axEff, fs./1e9, 100*efficiency11);
        addlegendentry('D-plane 60\circ');
        ylim(axEff, [0 100]);
...
movelegend(figure(1), 'n');
movelegend(figure(2), 'n');
movelegend(figure(3), 'n');
