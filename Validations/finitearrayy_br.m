...
if(walled)
    touchstonefilepath = [touchstonefilepath, '_walled'];
end
...
return;
%%
[parameters, YCST] = CST.LoadData([touchstonefilepath, '.y2p']);
fsCST = parameters.frequencies;
figureex;
for(nyp = 1:2)
    hAx = subplot(2, 1, nyp);
    ...
    title(hAx, sprintf('Y1%i', nyp));
    plot(hAx, fsCST, real(squeeze(YCST(1,nyp,:))));
    addlegendentry(hAx, 'CST');
    plot(hAx, fsCST, imag(squeeze(YCST(1,nyp,:))), '--');

    plot(hAx, fs, real(squeeze(array.Ymat(1,nyp,:))));
    addlegendentry(hAx, 'Matlab');
    plot(hAx, fs, imag(squeeze(array.Ymat(1,nyp,:))), '--');
end
