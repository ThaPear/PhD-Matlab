...
ths = [eps:10:60, eps:10:60, eps:10:60, ] * pi/180;
phs = [0*ones(1,7), 45*ones(1,7), 90*ones(1,7)] * pi/180;
ifigs = [ones(1,7), 2*ones(1,7), 3*ones(1,7)];
...
%     dispex('Calculated input impedance in %.3fs.\n', toc(tc));
    ...
    [figS, axS] = a_figS(3*ifigs(iangle));
    [figZ, axZ] = a_figZ(3*ifigs(iangle)+1, fs, zfeed);
    [figVSWR, axVSWR] = a_figVSWR(3*ifigs(iangle)+2);
