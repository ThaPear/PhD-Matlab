...
dslot = 0.3e-3;
wfeed = 0.3e-3;
wslot = 0.8e-3; shortcore = 1; patch_area = 1.2;                            % -15.44dB & -6.57dB  % Lossless -15.44dB & -6.49dB
...
for(iangle = 1:length(ths))
    ...
    [figS, axS] = a_figS(1);
    [figZ, axZ] = a_figZ(2, fs, zfeed);
    [figVSWR, axVSWR] = a_figVSWR(3);
    ...
    [figL, axL] = a_fig(9);
        plot(axL, fsmetal2, 10*log10(1-(Lmetal2+Ldiel2)./0.5));
        if(iangle == 2)
            plot(axL, fsmetal4, 10*log10(1-(Lmetal4+Ldiel4)./0.5));
        end
        xlim(axL, [12 32]);
        ylim(axL, [-0.1 0]);
        xlabel('Frequency [GHz]');
        ylabel(axL, 'Losses [dB]');
        drawnow;
end
legend(axVSWR, axVSWR.Children(4:-1:1), {'Broadside', 'H-plane 60\circ', 'E-plane 60\circ', 'D-plane 60\circ'});
movelegend(axVSWR, 'n');
legend(axL, axL.Children(4:-1:1), {'Broadside', 'H-plane 60\circ', 'E-plane 60\circ', 'D-plane 60\circ'});
%%
